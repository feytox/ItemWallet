...
import static name.uwu.feytox.itemwallet.gui.CounterHUD.getXFromCenter;
import static name.uwu.feytox.itemwallet.gui.CounterHUD.getYFromCenter;
...
public class CounterEditor {
        ...
        for (CounterEditor editor : availableEditors) {
            if (editor.x1 <= click_x && click_x <= editor.x2) {
                if (editor.y1 <= click_y && click_y <= editor.y2) {
                    activeEditor = editor;
                    ItemWalletClient.playSound(SoundEvents.UI_BUTTON_CLICK.value());
                    return;
                }
            }
        }
    ...
    public static void save() {
        ModConfig.save();
        activeEditor = null;
        ItemWalletClient.playSound(SoundEvents.ENTITY_VILLAGER_YES);
    }

    public static void discard() {
        ModConfig.load();
        activeEditor = null;
        ItemWalletClient.playSound(SoundEvents.ENTITY_VILLAGER_NO);
    }
    ...
    public void genSaveConsumer(ScreenType screenType, EditorLineInfo line_info) {
        ...
        if (!config.simpleMode) {
            ...
            switch (screenType) {
                case INVENTORY -> {
                    // доп. проверка нужна для последующей реализации SP Wallet
